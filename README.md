...
Go doesn't have classes. However you can defince methods on types.

`METHOD` - A method is a fuction with special reciever argument. The receiver appears in its own arguments list between `func` keyword and the method `name`.

The code given below defines a type using struct for a Square. The it defines methods to find area, perimeter and diagonal of square. 
...
When You want to change the value of the receiver from your method. You can construct the method with pointer 
receiver. 
...
If you remove the pointer receiver from the Scale method 
